...
export class AStarSearch extends BaseSearch {
    ...
    updateFrontier(node, goal) {
        let nodeIndex = this.frontier.findIndex((frontier) => {
            let valueFrontier = frontier.pathCost + frontier.state.distance(goal),
                valueNode = node.pathCost + node.state.distance(goal);
            return frontier.state.id === node.state.id && valueFrontier > valueNode;
        });
        if (nodeIndex > 0) {
